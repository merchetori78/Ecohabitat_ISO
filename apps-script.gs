...
// ============================================================
// INSTRUCCIONES:
// 1. Google Sheet preconfigurada para EcoHabitat
// 2. Ve a Extensiones → Apps Script
// 3. Pega este código y guarda
// 4. Despliega → Nueva implementación → Web App
//    - Ejecutar como: Yo
//    - Quién tiene acceso: Cualquier persona
// 5. La app ya incluye la URL del Web App preconfigurada
// ============================================================

const SHEET_ID = '1IlgaOGnzS1lClFJ0wpVRwK-ZgDL-i_FDRopEozYKbvU';
  ...
  try {
    ...
    if (data.action === 'sync') {
      ...
      if (data.tipo === 'inventario') {
        ...
        sheet.appendRow(['Fecha', 'Máquina', 'Categoría', 'Tipo Mantenimiento', 'Tareas Realizadas', 'Técnico', 'Almacén', 'Observaciones', 'Incidencia', 'Riesgo Ambiental', 'Acción Correctiva/Preventiva']);
        data.datos.forEach(d => {
          sheet.appendRow([
            d.fecha || '',
            d.maquina || '',
            ...
            d.observaciones || '',
            d.incidencia || 'No',
            d.riesgoAmbiental || 'No aplica',
            d.accionCorrectiva || ''
          ]);
        });
        formatearCabecera(sheet, 11);
        // Colorear por tipo
        const tipoColores = {
          'Diario': '#d4f0d9',
        ...
        rows2.forEach((row, i) => {
          ...
          sheet.getRange(i + 1, 1, 1, 11).setBackground(color);
        });
      }
